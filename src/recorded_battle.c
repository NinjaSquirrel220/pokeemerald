            ...
            else
            {
                if (sBattleRecords[battlerId][sRecordedBytesNo[battlerId]] == ACTION_MOVE_CHANGE)
                {
                    ...
                    for (j = 0; j < MAX_MON_MOVES; j++)
                    {
                        ...
                        array2[j] = (MonDisableStruct(battlerId)->mimickedMoves & gBitTable[j]) >> j;
                    }
                    for (j = 0; j < MAX_MON_MOVES; j++)
                    {
                    ...
                    MonDisableStruct(battlerId)->mimickedMoves = 0;
                    for (j = 0; j < MAX_MON_MOVES; j++)
                    {
                        gBattleMons[battlerId].ppBonuses |= (array3[j]) << (j << 1);
                        MonDisableStruct(battlerId)->mimickedMoves |= (array2[j]) << (j);
                    }
