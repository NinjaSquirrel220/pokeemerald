...
static void BattleStartClearSetData(void)
{
    ...
    {
        gStatuses3[i] = 0;
        gBattleMons[i].disableStruct.isFirstTurn = 2;
        gLastMoves[i] = 0;
...
void SwitchInClearSetData(void)
{
    ...
    if (gBattleMoves[gCurrentMove].effect != EFFECT_BATON_PASS)
    {
        ...
        {
            if ((gBattleMons[i].status2 & STATUS2_ESCAPE_PREVENTION) && gBattleMons[i].disableStruct.battlerPreventingEscape == gActiveBattler)
                gBattleMons[i].status2 &= ~STATUS2_ESCAPE_PREVENTION;
            if ((gStatuses3[i] & STATUS3_ALWAYS_HITS) && gBattleMons[i].disableStruct.battlerWithSureHit == gActiveBattler)
            {
                gStatuses3[i] &= ~STATUS3_ALWAYS_HITS;
                gBattleMons[i].disableStruct.battlerWithSureHit = 0;
            }
        }
    }
    if (gBattleMoves[gCurrentMove].effect == EFFECT_BATON_PASS)
    {
        ...
        for (i = 0; i < gBattlersCount; i++)
        {
            if (GetBattlerSide(gActiveBattler) != GetBattlerSide(i)
             ...
             && (gBattleMons[i].disableStruct.battlerWithSureHit == gActiveBattler))
            {
                gStatuses3[i] &= ~(STATUS3_ALWAYS_HITS);
                gStatuses3[i] |= 0x10;
            }
    ...
    memset(&gBattleMons[gActiveBattler].disableStruct, 0, sizeof(struct DisableStruct));

    if (gBattleMoves[gCurrentMove].effect == EFFECT_BATON_PASS)
    {
        gBattleMons[gActiveBattler].disableStruct.substituteHP = disableStructCopy.substituteHP;
        gBattleMons[gActiveBattler].disableStruct.battlerWithSureHit = disableStructCopy.battlerWithSureHit;
        gBattleMons[gActiveBattler].disableStruct.perishSongTimer = disableStructCopy.perishSongTimer;
        gBattleMons[gActiveBattler].disableStruct.perishSongTimerStartValue = disableStructCopy.perishSongTimerStartValue;
        gBattleMons[gActiveBattler].disableStruct.battlerPreventingEscape = disableStructCopy.battlerPreventingEscape;
    }

    gMoveResultFlags = 0;
    gBattleMons[gActiveBattler].disableStruct.isFirstTurn = 2;
    gBattleMons[gActiveBattler].disableStruct.truantSwitchInHack = disableStructCopy.truantSwitchInHack;
...
void FaintClearSetData(void)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if ((gBattleMons[i].status2 & STATUS2_ESCAPE_PREVENTION) && gBattleMons[i].disableStruct.battlerPreventingEscape == gActiveBattler)
            gBattleMons[i].status2 &= ~STATUS2_ESCAPE_PREVENTION;
        if (gBattleMons[i].disableStruct.skyDrop && gBattleMons[i].disableStruct.skyDropTrappingBattler == gActiveBattler)
            gBattleMons[i].disableStruct.skyDrop = 0;
			gBattleMons[i].disableStruct.skyDropTrappingBattler = 0;
        ...
        case STATE_WAIT_ACTION_CHOSEN: // Try to perform an action.
            if (!(gBattleControllerExecFlags & ((gBitTable[gActiveBattler]) | (0xF0000000) | (gBitTable[gActiveBattler] << 4) | (gBitTable[gActiveBattler] << 8) | (gBitTable[gActiveBattler] << 0xC))))
            {
                ...
                case B_ACTION_USE_MOVE:
                    ...
                    else if (gBattleMons[gActiveBattler].disableStruct.encoredMove != 0)
                    {
                        gChosenMoveByBattler[gActiveBattler] = gBattleMons[gActiveBattler].disableStruct.encoredMove;
                        *(gBattleStruct->chosenMovePositions + gActiveBattler) = gBattleMons[gActiveBattler].disableStruct.encoredMovePos;
                        gBattleCommunication[gActiveBattler] = STATE_WAIT_ACTION_CONFIRMED_STANDBY;
                        return;
                    }
                    else
                ...
                case B_ACTION_SWITCH:
                    *(gBattleStruct->field_58 + gActiveBattler) = gBattlerPartyIndexes[gActiveBattler];
                    if (gBattleMons[gActiveBattler].status2 & (STATUS2_WRAPPED | STATUS2_ESCAPE_PREVENTION)
                        || gBattleMons[gActiveBattler].disableStruct.skyDrop
                    ...
                    else if (gChosenActionByBattler[GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(gActiveBattler)))] == B_ACTION_RUN)
                    {
                        RecordedBattle_ClearBattlerAction(GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(gActiveBattler))), 1);
                             ...
                             && (gProtectStructs[GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(gActiveBattler)))].noValidMoves
                                || gBattleMons[GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(gActiveBattler)))].disableStruct.encoredMove))
                    {
                        RecordedBattle_ClearBattlerAction(GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(gActiveBattler))), 1);
                    }
...
s8 GetMovePriority(u32 battlerId, u16 move)
{
    ...
    if (GetBattlerAbility(battlerId) == ABILITY_GALE_WINGS
        ...
        && (B_GALE_WINGS == GEN_6 || GetScaledHPFraction(gBattleMons[battlerId].hp, gBattleMons[battlerId].maxHP, 100) > 50))
    {
        priority++;
    }
...
static void TurnValuesCleanUp(bool8 var0)
{
    ...
    for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++)
    {
        ...
        else
        {
            ...
            if (gBattleMons[gActiveBattler].disableStruct.isFirstTurn)
                gBattleMons[gActiveBattler].disableStruct.isFirstTurn--;
    ...
    gSideStatuses[0] &= ~(SIDE_STATUS_QUICK_GUARD | SIDE_STATUS_WIDE_GUARD | SIDE_STATUS_MAT_BLOCK);
    gSideStatuses[1] &= ~(SIDE_STATUS_QUICK_GUARD | SIDE_STATUS_WIDE_GUARD | SIDE_STATUS_MAT_BLOCK);
...
static void CheckFocusPunch_ClearVarsBeforeTurnStarts(void)
{
    ...
    if (!(gHitMarker & HITMARKER_RUN))
    {
        while (gBattleStruct->focusPunchBattlerId < gBattlersCount)
        {
            ...
            if (gBattleMoves[gChosenMoveByBattler[gActiveBattler]].effect == EFFECT_FOCUS_PUNCH
                && !(gBattleMons[gActiveBattler].status1 & STATUS1_SLEEP)
                && !(gBattleMons[gBattlerAttacker].disableStruct.truantCounter)
                && !(gProtectStructs[gActiveBattler].noValidMoves))
            {
...
static void HandleAction_UseMove(void)
{
    ...
    else if (gBattleMons[gBattlerAttacker].disableStruct.encoredMove != MOVE_NONE
             && gBattleMons[gBattlerAttacker].disableStruct.encoredMove == gBattleMons[gBattlerAttacker].moves[gBattleMons[gBattlerAttacker].disableStruct.encoredMovePos])
    {
        gCurrentMove = gChosenMove = gBattleMons[gBattlerAttacker].disableStruct.encoredMove;
        gCurrMovePos = gChosenMovePos = gBattleMons[gBattlerAttacker].disableStruct.encoredMovePos;
             ...
             && gBattleMons[gBattlerAttacker].disableStruct.encoredMove != gBattleMons[gBattlerAttacker].moves[gBattleMons[gBattlerAttacker].disableStruct.encoredMovePos])
    {
        ...
        gBattleMons[gBattlerAttacker].disableStruct.encoredMove = MOVE_NONE;
        gBattleMons[gBattlerAttacker].disableStruct.encoredMovePos = 0;
        gBattleMons[gBattlerAttacker].disableStruct.encoreTimer = 0;
        *(gBattleStruct->moveTarget + gBattlerAttacker) = GetMoveTarget(gCurrentMove, 0);
    }
